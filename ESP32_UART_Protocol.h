...
struct SensorPacket
{
    ...
    uint16_t temperature; // 0-4096: temperature (maps to 3°C - 160°C)
    uint16_t pressure;    // 0-4096: pressure (maps to 0 - 1000 bar = ~0-10000m depth)
    uint16_t ph;          // 0-4096: solution pH (maps to 0 - 14; optimum for life ~10)
...
#define TEMP_MAX_CELSIUS 160.0f // Maximum temperature in Celsius (hydrothermal vent range)
#define PRESSURE_MIN 0.0f       // Minimum pressure (bar)
#define PRESSURE_MAX 1000.0f    // Maximum pressure (bar, ~10,000m depth)
#define PH_MIN 0.0f             // Minimum pH (strongly acidic)
#define PH_MAX 14.0f            // Maximum pH (strongly basic; optimum for life ~10)
...
inline void sendSensorPacket(const SensorPacket &packet)
{
    ...
    Serial.write((packet.ph >> 8) & 0xFF);
    Serial.write(packet.ph & 0xFF);
...
/**
 * Convert real-world pH to sensor value
 */
inline uint16_t phToSensorValue(float ph)
{
    ph = constrain(ph, PH_MIN, PH_MAX);
    return MAP_TO_SENSOR_RANGE(ph, PH_MIN, PH_MAX);
}
...
void loop() {
    ...
    if (millis() - lastSensorSend >= SENSOR_INTERVAL_MS) {
        ...
        sensorPacket.ph = phToSensorValue(readPHSensor());
