...
fn lightningBolt(uv: vec2<f32>, time: f32, boltId: f32, timeInCycle: f32) -> f32 {
    ...
    if (edgeRand < 0.25) {
        startPos = vec2<f32>(0.1 + posRand * 0.8, 0.05);
        // Top edge
        direction = vec2<f32>(0.0, 1.0);
        // Point downward
    }
    else if (edgeRand < 0.5) {
        startPos = vec2<f32>(0.95, 0.1 + posRand * 0.8);
        // Right edge
        direction = vec2<f32>(- 1.0, 0.0);
        // Point left
    }
    else if (edgeRand < 0.75) {
        startPos = vec2<f32>(0.1 + posRand * 0.8, 0.95);
        // Bottom edge
        direction = vec2<f32>(0.0, - 1.0);
        // Point up
    }
    else {
        startPos = vec2<f32>(0.05, 0.1 + posRand * 0.8);
        // Left edge
        direction = vec2<f32>(1.0, 0.0);
        // Point right
    }
    ...
    let segmentInterval = 0.5;
    // New segment every 0.5 seconds
    let segmentDuration = 2.0;
    // Each segment visible for 2 seconds
...
fn main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // Get electrical activity from attraction scale
    let electricalActivity = sim_params.interTypeAttractionScale;

    // Lightning appears when electrical activity > 1.0
    if (electricalActivity <= 1.0) {
        return vec4<f32>(0.0, 0.0, 0.0, 0.0);
    }

    // Lightning flash every 3 seconds for testing
    let flashInterval = 3.0;
    let timeInCycle = sim_params.time % flashInterval;
    let flashDuration = 10.0;
    // 10 seconds to build the full bolt
    ...
    let alpha = boltIntensity * (electricalActivity - 1.0) * sim_params.lightningIntensity;
