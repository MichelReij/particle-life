...
fn calculate_edge_damping_factor(pos: vec2<f32>) -> f32 {
    ...
    if (abs(sim_params.drift_x_per_second) < 0.1) {
        ...
        // Always include vertical edges so particles don't accumulate at top/bottom.
        min_edge_dist = min(dist_right, min(dist_top, dist_bottom));
    }
    else {
        ...
        // Always include vertical edges so particles don't accumulate at top/bottom.
        min_edge_dist = min(dist_left, min(dist_top, dist_bottom));
    }
...
// Exclusive again: HTV never spawns 8-10, WLP never spawns 5-7. An earlier
// version let a little weight leak across that boundary (hoping it would
